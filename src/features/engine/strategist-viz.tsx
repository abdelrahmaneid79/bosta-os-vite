...
export const VIZ_PALETTE = ["var(--mag)", "var(--violet)", "var(--cyan)", "var(--green)", "var(--amber)", "var(--teal)"];
...
export function HealthRadar({ categories, overall, status, size = 220 }: {
  ...
  const col = overall == null ? "var(--faint)" : overall >= 75 ? "var(--green)" : overall >= 55 ? "var(--amber)" : "var(--red)";
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size}>
        ...
        {cats.map((c, i) => {
          ...
          return <text key={i} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fontSize={9.5} fontWeight={700} fill="var(--muted)">{c.label}</text>;
        })}
        ...
        <text x={cx} y={cy + 11} textAnchor="middle" fontSize={7} fontWeight={700} fill="var(--dim)">/ 100</text>
      </svg>
...
export function Donut({ segments, centerValue, centerLabel, size = 180 }: {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size}>
      ...
      <text x={cx} y={cy - 4} textAnchor="middle" fontSize={size * 0.13} fontWeight={800} fill="var(--text)">{centerValue}</text>
      <text x={cx} y={cy + 14} textAnchor="middle" fontSize={size * 0.06} fontWeight={600} fill="var(--dim)">{centerLabel}</text>
    </svg>
  );
}
...
export function Area({ data, height = 150, color = "var(--mag)" }: { data: number[]; height?: number; color?: string }) {
  const w = 600, pad = 6;
  if (!data.length) return <div style={{ height, color: "var(--faint)", fontSize: 12 }}>No data</div>;
...
export function Bars({ data, color = "var(--violet)", fmt = String }: { data: { label: string; value: number }[]; color?: string; fmt?: (n: number) => string }) {
  const max = Math.max(1, ...data.map((d) => d.value));
  return (
    <div style={{ display: "grid", gap: 9 }}>
      {data.map((d) => (
        <div key={d.label} style={{ display: "grid", gridTemplateColumns: "84px 1fr auto", gap: 10, alignItems: "center" }}>
          <span style={{ fontSize: 12, color: "var(--muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} dir="auto">{d.label}</span>
          ...
          <span style={{ fontSize: 11.5, color: "var(--dim)", fontVariantNumeric: "tabular-nums" }}>{fmt(d.value)}</span>
        </div>
      ))}
    </div>
  );
