...
export async function readDayReportPhoto(file: File): Promise<RawDayReport> {
  ...
  if (error) {
    ...
    if (ctx?.status === 401) throw new DayReportAuthError("The reader rejected the session — sign in again.");
    // Surface the edge function's actual error (e.g. Anthropic out of credits)
    // so the owner sees the real reason instead of a generic failure.
    let reason = "";
    try { const body = await ctx?.clone().json(); reason = body?.detail || body?.error || ""; } catch { /* body already read / not json */ }
    throw new Error(reason ? `Reader error — ${reason}`.slice(0, 300) : (error as Error).message || "Reader failed");
  }
